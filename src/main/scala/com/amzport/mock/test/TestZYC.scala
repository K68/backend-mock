...
//import pb.baccaratwrapper.WrapperDragonTigerMessage
...
object TestZYC extends App {
  ...
  MockWeb.observe("1014", (_, m) =>
    m("msg") match {
      //用户进入龙虎斗
      case "DragonTigerUserSitDownReceive" =>  //URL: http://127.0.0.1:9999/1012?msg=DragonTigerUserSitDownReceive&userId=?&roomId=?
        val userId = m("userId")
        val roomId = m("roomId")
        MockLog.debug(s"${userId.toString}", 1012)
        if(userId.toInt == 6) {
        ...
        Mock1012.sendRoomDispatch(FlowMeta(1012, userId, 1, roomId, 2), UserSitDownReceive)
      //用户申请庄家
      case "DragonTigerApplyBankerReceive" =>  //URL: http://127.0.0.1:9999/1012?msg=DragonTigerApplyBankerReceive&userId=?&roomId=?
        val userId = m("userId")
        val roomId = m("roomId")
        val ApplyBankerReceive = WrapperBaccaratMessage().withApplyBankerReceive(pb.baccarat.ApplyBankerReceive(userId.toLong))
        Mock1012.sendRoomDispatch(FlowMeta(1012, userId, 1, roomId, 2), ApplyBankerReceive)
      //用户下庄
      case "DragonTigerCancelBanker" =>  //URL: http://127.0.0.1:9999/1012?msg=DragonTigerCancelBanker&userId=?&roomId=?&ChairID=?&ListApplyCount=?
        val userId = m("userId")
        val roomId = m("roomId")
        val CancelBanker = WrapperBaccaratMessage().withCancelBanker(pb.baccarat.CancelBanker(userId.toLong))
        Mock1012.sendRoomDispatch(FlowMeta(1012, userId, 1, roomId, 2), CancelBanker)
      //用户下注
      case "DragonTigerPlaceBetReceive" =>  //URL: http://127.0.0.1:9999/1012?msg=DragonTigerPlaceBetReceive&userId=?&roomId=?&ChairID=?&BetArea=?&BetScore=?&AndroidUser=?
        val userId = m("userId")
        val roomId = m("roomId")
        val ChairID = m("ChairID")
        val BetArea = m("BetArea")
        ...
        Mock1012.sendRoomDispatch(FlowMeta(1012, userId, 1, roomId, 2), PlaceBetReceive)
      //用户退出百家乐
      case "DragonTigerUserStandUp" =>  //URL: http://127.0.0.1:9999/1012?msg=DragonTigerUserStandUp&userId=?&roomId=?&ChairID=?
        MockLog.debug(s"${m.toString}", 1012)
        val ChairID = m("ChairID")
        val userId = m("userId")
        val roomId = m("roomId")
