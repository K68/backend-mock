...
import pb.dice.WrapperDiceMessage.Msg
import scalapb.GeneratedMessage

object Mock1016 {

  def sendRoomDispatch(flowMeta: FlowMeta, m: WrapperDiceMessage): Unit = {
  ...
  private def receiveMessage(space: Int, flowMeta: FlowMeta, value: ByteString): Unit = {
    MPB.parseWrap(WrapperDiceMessage, value)  match {
      case Some(wrapper) =>
        wrapper.msg match {
  ...
  def observe(cb: (FlowMeta, GeneratedMessage) => Unit): Unit = {
    callbackOpt = Some(cb)
    MockUser.observe(1016, receiveMessage)
  }
