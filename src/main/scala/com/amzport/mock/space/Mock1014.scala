...
import pb.dragonTigerwrapper.WrapperDragonTigerMessage.Msg
import scalapb.GeneratedMessage

object Mock1014 {

  def sendRoomDispatch(flowMeta: FlowMeta, m: WrapperDragonTigerMessage): Unit = {
    MockLog.info(s"进入龙虎斗：${flowMeta.fromId} ${flowMeta.toId}", flowMeta.space)
  ...
  private def receiveMessage(space: Int, flowMeta: FlowMeta, value: ByteString): Unit = {
    MPB.parseWrap(WrapperDragonTigerMessage, value)  match {
      case Some(wrapper) =>
        wrapper.msg match {
  ...
  def observe(cb: (FlowMeta, GeneratedMessage) => Unit): Unit = {
    callbackOpt = Some(cb)
    MockUser.observe(1014, receiveMessage)
  }
